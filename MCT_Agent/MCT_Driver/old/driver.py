...
## CLASSES                                                                   ##
###############################################################################
class MCT_Communication_Temp(Process):
    ...
    ** get_resources_inf == get MCT resouces information.
    ...
    def __waiting_return(self, requestId):
        ...
        while True or count < REQUEST_PENDING_MAXTRY:
            ...
            if dataReceived != []:
                ## LOG:
                LOG.info("[MCT_AGENT] DATA RECEIVED %s", dataReceived[0][0]);
            ...
            ## LOG:
            LOG.info("[MCT_AGENT] WAITING FOR RETURN FROM s", dataReceived);
